...
\begin{frame}[t]
	...
	\begin{tikzpicture}[remember picture,overlay]
			...
			\node[anchor=south] at ($(current page.south) + (0, 0.4)$) {
				...
				\only<2>{\includegraphics[width=\textwidth]{improved-unet-2.pdf}}%
				\only<3>{\includegraphics[width=\textwidth]{improved-unet-3.pdf}}%
			};
			% Adjust the position of the text
			\node[align=center] at ($(current page.center) + (0, 2.5)$) {
				\only<1>{Separable Convolutions \\ $7 \times 7$ kernels \\ Inverse
				Bottleneck}%
				\only<2>{\yellow{Batch Normalization}}%
				\only<3>{\purple{Additive skip connections}}
			};
	\end{tikzpicture}
