...
\begin{frame}
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			% Input Image
			\node[draw, rectangle, minimum width=1.5cm, minimum height=1.5cm] (input)
			at (0, 0) {Input Image};
...
\begin{frame}[t]
	...
	\vspace{-2ex}
		...
		\begin{figure}
			\centering
			\only<1>{\includegraphics[width=0.9\textwidth]{segmentation-prediction.png}}
			\only<2->{\includegraphics[width=0.9\textwidth]{segmentation-bin-prediction.png}}
			\includegraphics[width=0.9\textwidth]{segmentation-ground-truth.png}
		\end{figure}
